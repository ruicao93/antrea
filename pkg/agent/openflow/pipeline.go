...
func (c *client) hostBridgeUplinkFlows(uplinkPort uint32, bridgeLocalPort uint32, category cookie.Category) (flows []binding.Flow) {
	flows = []binding.Flow{
		c.pipeline[uplinkTable].BuildFlow(priorityMiss).MatchInPort(uplinkPort).
...
// loadBalancerServiceFromOutsideFlow generates the flow to forward LoadBalancer service traffic from outside
...
func generateBasicPipeline(bridge binding.Bridge, enableProxy bool) map[binding.TableIDType]binding.Table {
	if enableProxy {
